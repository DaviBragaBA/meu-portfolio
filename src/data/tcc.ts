...
export const tcc = {
  ...
  abstract:
    ...
    "A conclusão destaca a comparação entre os métodos implementados (em C++, com instâncias do repositório 0/1 Knapsack e resolvedor CPLEX), mostrando que algoritmos guloso e aproximado são opções eficientes para instâncias grandes, com soluções próximas do ótimo.",
  ...
  sections: [
    ...
    },
    {
      ...
      content:
        "Os algoritmos guloso e aproximado são muito mais rápidos que os métodos exatos (programação dinâmica e PLI), com soluções próximas da ótima nas instâncias testadas. O algoritmo guloso é o mais rápido; o aproximado é um pouco mais lento mas oferece garantia de pelo menos 50% do ótimo. A escolha do algoritmo depende do trade-off entre tempo de execução e qualidade da solução. O trabalho conclui que a Teoria dos Jogos, o design de mecanismos e a aplicação de algoritmos específicos são fundamentais para o desenvolvimento de regras e mecanismos em contextos estratégicos, como leilões e otimização combinatória.",
    },
  ],
  ...
  /** Imagens do TCC: coloque arquivos em public/tcc/ e adicione aqui. Ex.: figuras do Paradoxo de Braess, Pedra-Papel-Tesoura, curvas de alocação, tabelas. */
  images: [] as TccImage[],
};
